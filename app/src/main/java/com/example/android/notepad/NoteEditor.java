...
public class NoteEditor extends Activity {
    ...
    private boolean isNightModeEnabled() {
        ...
        return sharedPreferences.getBoolean("NightMode", false);  // 默认为白天模式
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 获取当前的模式（可以从 SharedPreferences 或者其它方式获取）
    ...
    private void setDayMode() {
        // 设置背景为白色背景
        findViewById(android.R.id.content).setBackgroundColor(Color.parseColor("#FFFFFF")); // 白色背景
        // 设置输入框背景色为白色
        LinedEditText editText = (LinedEditText) findViewById(R.id.note); // 强制转换为 LinedEditText
        if (editText != null) {
            editText.setBackgroundColor(Color.WHITE);  // 设置输入框背景色为白色
            editText.invalidate();  // 强制重新绘制
        }
    }

    private void setNightMode() {
        // 设置背景为浅灰色背景
        findViewById(android.R.id.content).setBackgroundColor(Color.parseColor("#E0E0E0")); // 浅灰色背景
        // 设置输入框背景色为稍微深一点的灰色
        LinedEditText editText = (LinedEditText) findViewById(R.id.note); // 强制转换为 LinedEditText
        if (editText != null) {
            editText.setBackgroundColor(Color.parseColor("#B0B0B0"));  // 输入框背景色为更淡的灰色
            editText.invalidate();  // 强制重新绘制
        }
    }

    private void setTextColorForAllViews(int color) {
        // 如果不需要修改字体颜色，可以去掉这个方法
        // 保持原有字体颜色不变
    }
